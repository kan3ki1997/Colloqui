...
public class IntervistatoreController {
    ...
    public String doLogin(@RequestBody IntervistatoreDTO intervistatore) {
        ...
        if (intervistatoreDTO != null) {
            return intervistatoreDTO.getUsername().concat(" loggato");
        }
    ...
    public void refreshToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            try {
                String refreshToken = authorizationHeader.substring("Bearer ".length()); // lettere da rimuovere
                ...
                IntervistatoreDTO intervistatoreDTO = intervistatoreService.getUserByUsername(username);

                String accessToken = JWT.create()
                        .withSubject(intervistatoreDTO.getUsername())
                        .withExpiresAt(new Date(System.currentTimeMillis() + 1000 * 60)) // scadenza token di 10 minuti
                        .withIssuer(request.getRequestURL().toString())
                        .withClaim("role", intervistatoreDTO.getRole().getNome())
                        .sign(algorithm);
